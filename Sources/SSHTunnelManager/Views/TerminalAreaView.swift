...
private struct WorkspacePill: View {
    ...
    var body: some View {
        ...
        .contextMenu {
            Button {
                ConnectionHealthWindowController.open(
                    key: "ws-\(workspace.id.uuidString)",
                    title: workspace.name,
                    provider: { TerminalSessionManager.shared.openSessions(inWorkspace: workspace.id) })
            } label: { Label("Connection Health…", systemImage: "waveform.path.ecg") }
            Divider()
...
private struct TerminalTabContextMenu: View {
    ...
    var body: some View {
        // Live connection health opens in its own window (networked tabs only).
        if sessions.probeEndpoint(for: session) != nil {
            Button {
                ConnectionHealthWindowController.open(
                    key: "tab-\(session.id.uuidString)",
                    title: session.title,
                    provider: { [weak session] in session.map { [$0] } ?? [] })
            } label: { Label("Connection Health…", systemImage: "waveform.path.ecg") }
            Divider()
        }

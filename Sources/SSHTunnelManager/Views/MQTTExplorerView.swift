...
struct MQTTExplorerView: View {
    ...
    /// The graph pane: the shared numeric graph fed by this topic's retained
    /// history.
    private func graphSection(topic: String) -> some View {
        let samples = (client.history[topic] ?? []).map {
            NumericGraphSample(time: $0.time, values: $0.values)
        }
